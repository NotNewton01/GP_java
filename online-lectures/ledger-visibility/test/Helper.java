...
public class Helper {
	...
	protected Field checkField(Class<?> testClass, String field, Class<?> type, boolean priv) {
		try {
			Field declaredField = testClass.getDeclaredField(field);
			assertTrue("Attribut " + testClass.getSimpleName() + "." + field + " sollte den Typ " + type + " haben",
					declaredField.getType().equals(type));
			if(priv)
				assertTrue("Attribut " + testClass.getSimpleName() + "." + field + " sollte privat sein", Modifier.isPrivate(declaredField.getModifiers()));
			assertTrue("Attribut " + testClass.getSimpleName() + "." + field + " sollte nicht static sein",
					!Modifier.isStatic(declaredField.getModifiers()));
			assertTrue("Attribut " + testClass.getSimpleName() + "." + field + " sollte nicht final sein",
